## Spike: can Nim produce ATProto-compatible ES256 / DPoP proofs?
##
## This is the riskiest part of a Nim port. Glean's Go backend gets OAuth from
## bluesky-social/indigo; Nim has no ATProto library, so the port would have to
## build the DPoP layer itself. Everything downstream of sign-in depends on it,
## so it is worth proving before committing to the rest.
##
...
## DER unwrapping step to get wrong.
##
## Self-verification would only prove internal consistency, so the signed token
## is handed to a Go verifier (verify/verify.go) built on the same stdlib the
## real server uses.
